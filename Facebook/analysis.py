...
def weekly_top_five_names_per_title(df):
    ...
    google_sheet_id = "14hYsDp8ef4P1LuPeqsUe4ok5eiSuHeNNqc0eMQLI1Lo"
...
def aggregate_and_save(df, file_name, folder_location):
    ...
    aggregated_df = df.groupby('title').agg({
        'actual.likeCount': 'sum',
        'actual.shareCount': 'sum',
        'actual.commentCount': 'sum'
    }).reset_index()

    # Calculate the total engagement for each row
    aggregated_df['TotalEngagement'] = aggregated_df['actual.likeCount'] + aggregated_df['actual.shareCount'] + aggregated_df['actual.commentCount']

    # Compute the percentages
    aggregated_df['Likes'] = (aggregated_df['actual.likeCount'] / aggregated_df['TotalEngagement']) * 100
    aggregated_df['Shares'] = (aggregated_df['actual.shareCount'] / aggregated_df['TotalEngagement']) * 100
    aggregated_df['Comments'] = (aggregated_df['actual.commentCount'] / aggregated_df['TotalEngagement']) * 100

    # Keep only the desired columns
    aggregated_df = aggregated_df[['title', 'Likes', 'Shares', 'Comments']]
...
folder_location = "1UldSEv6fsvjTqF2h0FskRZStc7-cSrLs"
...
theme_folder="1UcLEI7ap_tTfLMDo2-CNeBaYWbC_ZOU5"
